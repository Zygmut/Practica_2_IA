...
public class Cerca {
    ...
    public ArrayList<Punt> non_repeated_heuristica(ArrayList<Punt> target, ArrayList<Punt> toAdd, int tipus, Punt desti) {
        ...
        for (int i = 0; i < toAdd.size(); i++) {
            ...
            //si no repetido, cálculamos f(no) e insertamos
            if (!found) {
                Punt aux = toAdd.get(i);
                aux.distanciaDeLinici++;
                calculaF(aux,desti,tipus);
                System.out.println(aux.distanciaDeLinici+"G DE UN NO ENCONTRADO");
                non_repeated.add(aux);
            }
            //si esta repetido, mantenemos el de menor f(n)
            else if (found ){
    ...
    public Cami CercaAmbHeurística(Punt origen, Punt desti, int tipus) {   // Tipus pot ser MANHATTAN o EUCLIDIA
        ...
        current_punt.distanciaDeLinici = 0; //nodo inicial
        calculaF(current_punt,desti,tipus);
        System.out.println(current_punt.distanciaDeLinici+" LA G DEL PRIMERO");
        open.add(current_punt);

        while (open.size() != 0) {
            found = false;
            Punt node = getLeast(open,desti,tipus); //nodo con menor f(n)
            open.remove(0);

            //miramos que no este repetido en tancats
            int i;
    ...
    /**
     * Cálcula G como el nº de nodos reales del camino
     * @param origen Punto de origen
     * @param desti Punto de destino
     * @return nº de nodos del camino
